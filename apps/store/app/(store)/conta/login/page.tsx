 ...
 * Lê ?redirect para retornar ao destino após autenticação.
 */
...
/* ── Componente interno (usa useSearchParams → precisa de Suspense no pai) ── */

function LoginPageInner() {
  const router       = useRouter()
  const searchParams = useSearchParams()
  const redirectTo   = searchParams.get('redirect') ?? '/conta/pedidos'
  ...
  useEffect(() => {
    if (buyer) router.replace(redirectTo)
  ...
  return (
    <div className="flex min-h-[70vh] items-center justify-center px-4 py-10">
      <div className="w-full max-w-sm">
        ...
        {tab === 'login'
          ? <LoginForm onSuccess={handleSuccess} login={login} />
          : <RegisterForm tenantSlug={slug} onSuccess={handleSuccess} />
        }
      </div>
    </div>
  )
...
/* ── Página exportada — Suspense obrigatório para useSearchParams ── */
