...
import { parseUA } from '../../lib/parseUA.js'
...
masterAnalyticsRouter.get('/:slug/users', async (req, res) => {
  try {
    ...
    const activeSessions = await prisma.userSession.findMany({
      ...
      select: { userId: true, lastActivityAt: true, startedAt: true, ip: true, userAgent: true },
    })

    const sessionsByUser = {}
    for (const s of activeSessions) {
      if (!sessionsByUser[s.userId]) sessionsByUser[s.userId] = []
      sessionsByUser[s.userId].push({ ...s, device: parseUA(s.userAgent) })
    }
    ...
    res.json({
      ...
      users: users.map(u => ({
        ...
        lastDevice: (() => {
          ...
          const last = sessions.reduce((mx, s) => !mx || s.lastActivityAt > mx.lastActivityAt ? s : mx, null)
          return last?.device ?? null
        })(),
      })),
    })
  } catch (err) {
...
masterAnalyticsRouter.get('/:slug/sessions', async (req, res) => {
  try {
    ...
    res.json({
      data: sessions.map(s => ({ ...s, device: parseUA(s.userAgent) })),
...
masterAnalyticsRouter.get('/:slug/events', async (req, res) => {
  try {
    ...
    res.json({
      data: events.map(e => ({ ...e, device: parseUA(e.userAgent) })),
      meta: { page: pageN, limit: limitN, total, pages: Math.ceil(total / limitN) },
    })
  } catch (err) {
    if (err.status) return res.status(err.status).json({ error: err.message })
